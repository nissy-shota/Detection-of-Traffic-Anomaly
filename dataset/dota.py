...
class DoTADataset(data.Dataset):
    ...
    def get_input(self, ego_motion_session, start, end):
        ...
        if start == 0:
            return np.vstack(
                [
                    ego_motion_session[0:1, :] - ego_motion_session[0:1, :],
                    ego_motion_session[start + 1 : end]
                    - ego_motion_session[start : end - 1],
                ]
            )
        else:
            return (
                ego_motion_session[start:end, :]
                - ego_motion_session[start - 1 : end - 1, :]
            )
